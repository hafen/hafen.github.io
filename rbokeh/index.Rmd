...
navpills: |
  ...
  <li><a href='rd.html'>Function Ref</a></li>
  <li><a href='https://github.com/bokeh/rbokeh'>Github <i class='fa fa-github'></i></a></li>
---

# Background #

[Bokeh](http://bokeh.pydata.org) is a visualization library that provides a flexible and powerful declarative framework for creating web-based plots.  Bokeh renders plots using HTML canvas and provides many mechanisms for interactivity.  Bokeh has interfaces in Python, Scala, Julia, and now R.  Full acknowledgements to the [Bokeh team at Continuum Analytics](http://bokeh.pydata.org/en/latest/).

<span class="label label-success">Note:</span> This package has not been publicly announced and we'd like to keep it that way until we are sure we are settled on things.  So if you have come across this site, we encourage you to try it out and [provide feedback](), but to wait until the official announcement to share on social media.
...
Here since we are specifying color and glyph by `Species`, a legend is automatically created and the points are colored according to the default color scheme.
...
for(i in seq_along(splom_list)) {
  splom_list[[i]] <- figure(width = 200, height = 200, tools = tools) %>%
...
More control over axes will be added very soon so that adjacent panels in the grid can be closer together.
...
figure(xgrid = FALSE, ygrid = FALSE, width = 630, height = 500,
...
And for all those R cat lovers:

```{r, message=FALSE}
url <- "http://placekitten.com/g/600/600"
p <- figure(width = 600, height = 600, padding_factor = 0) %>%
  ly_image_url(url = url, w = 10, h = 10, anchor = "bottom_left")
p
```
...
figure(xlim = c(-0.1, 1.35), width = 550) %>%
...
Full access to the different parameters controlling axis attributes beyond the axis type will be added soon.
